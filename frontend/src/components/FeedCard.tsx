...
export default function FeedCard({ story, authorName, extraClass, edit, session }: Props) {
    if (story.id === 0) {
        return (<></>)
    }
        ...
        <>
            <Modal2 isOpen={modalVis} onClose={() => { setVisModal(!modalVis) }}>
                <form className='flex flex-col' onSubmit={(e) => { e.preventDefault() }}>
                    <h2 className='mb-5 font-semibold'>Change Visibility</h2>

                    <FeedCard story={story} extraClass='w-full' />

                    <fieldset className="my-5 flex flex-col">
                        <h5 className="text-(--yellow-500)">Visibility</h5>
                        <select className="bg-black px-3 py-2 my-2" name="visibility" onChange={(e) => { handleDetails(e) }}>
                    ...
                    <div className='flex w-full'>
                        <button className='btn ml-auto mr-2' onClick={() => { setVisModal(!modalVis) }}>Cancel</button>
                        <button className='btn yellow' onClick={() => { updateVisibility(story.id, vis); setVisModal(!modalVis) }}>Continue</button>
                    </div>
                </form>
            </Modal2>
            <Modal2 isOpen={deleteModal} onClose={() => { setDelete(!deleteModal) }}>
                <form className='flex flex-col items-center justify-center w-full h-full' onSubmit={(e) => { e.preventDefault() }}>
                    <h2 className='font-semibold text-center mb-6'>Are you sure you want to delete this story?</h2>
                    <FeedCard story={story} extraClass='w-full' />

                    <div className='flex mt-5 ml-auto items-center '>
                        <button className='mr-5 btn' onClick={() => { setDelete(!deleteModal) }}>Cancel</button>
                        <button className='btn yellow' onClick={() => { DeleteStory(story.id, session) }}>
                            Delete <img src={warning} alt='delete' />
                        </button>
                    </div>
                </form>
            </Modal2>
