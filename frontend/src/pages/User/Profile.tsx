                ...
                <main className='flex m-auto w-[95%] h-[95%] bg-(--dark-200) p-5 rounded-2xl'>
                    <section className='flex flex-col h-full w-[20%] '>
                        <article className='flex flex-col h-[60%] items-center'>
                            ...
                            <h4 className='font-bold'>@Usuario.exe</h4>
                    ...
                    <section className='w-[80%] h-full'>
                        <nav className='flex h-[6%]'>
                            <div className={`px-4 py-2 ${tab === 0 ? 'bg-(--dark-400)' : 'bg-(--dark-800) text-(--gray)'} cursor-pointer  rounded-t-md transition-all ease-in-out duration-150`}
                                onClick={() => { setTab(0) }}>Tab 1</div>
                            <div className={`px-4 py-2 ${tab === 1 ? 'bg-(--dark-400)' : 'bg-(--dark-600) text-(--gray)'} cursor-pointer rounded-t-md`}
                                onClick={() => { setTab(1) }}>Tab 2</div>
                        </nav>
                        <article className='grid grid-cols-2 gap-5 p-2 bg-(--dark-400) w-full h-[90%]'>
                            {tab === 0 ? lista1.map((current, index) => (
                                <div className='bg-(--dark-200) transition-all ease-in-out duration-150'>Contenido {index}</div>
